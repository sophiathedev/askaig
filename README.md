...
- **Correction history** — two per-side running estimates (keyed by the **pawn skeleton** and by the **non-pawn material** configuration) of how far the static eval has historically sat from the search result; their sum nudges the static eval used for pruning and the quiescence stand-pat toward the truth (the raw, uncorrected eval is what's stored in the TT). Updated by the same gravity formula as the other history tables and persistent across moves.
