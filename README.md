...
- **Correction history** — a per-(side, pawn-structure) running estimate of how far the static eval has historically sat from the search result; it nudges the static eval used for pruning and the quiescence stand-pat toward the truth (the raw, uncorrected eval is what's stored in the TT). Updated by the same gravity formula as the other history tables and persistent across moves.
