...
// PeSTO's Evaluation Function — Ronald Friederich's (rofChade) tuned tapered piece-square tables.
// Unlike a single shared table, PeSTO carries a SEPARATE material value and piece-square table for
// the middlegame and the endgame, and interpolates between them by game phase. This namespace
// provides the per-piece middlegame/endgame score used by both the static evaluation and Position's
// incremental accumulators (`psqt_mg_score`/`psqt_eg_score`).
//
// Tables are written from White's view in the usual textbook layout (index 0 = a8 ... 63 = h1), so a
// White piece on Square sq reads [sq ^ 56] and a Black piece reads [sq] (the table mirrored
// vertically, which by symmetry yields the same positional value for the mirrored square).
namespace psqt {

  // Piece values used ONLY for move ordering (MVV-LVA) and quiescence delta pruning — not for the
  // evaluation itself, which uses the tapered MG_VALUE/EG_VALUE below.
  constexpr int VALUE[NPIECE_TYPES] = {100, 320, 330, 500, 900, 0};

  // PeSTO material values: middlegame and endgame (pawn, knight, bishop, rook, queen, king).
  constexpr int MG_VALUE[NPIECE_TYPES] = {82, 337, 365, 477, 1025, 0};
  constexpr int EG_VALUE[NPIECE_TYPES] = {94, 281, 297, 512, 936, 0};

  // Game phase: PHASE_MAX with all pieces on the board, decreasing as they are traded (knight=1,
  // bishop=1, rook=2, queen=4 per side -> 24). Used to interpolate between the middlegame and endgame.
  constexpr int PHASE_MAX = 24;

  // Middlegame piece-square tables (PeSTO), a8-first.
  constexpr int MG_TABLE[NPIECE_TYPES][NSQUARES] = {
          // PAWN
  ...
  // Endgame piece-square tables (PeSTO), a8-first.
  constexpr int EG_TABLE[NPIECE_TYPES][NSQUARES] = {
          // PAWN
  ...
  // White-perspective contribution (PeSTO material + PST) of piece `pc` on square `s`, in the
  // middlegame / endgame. `pc` must be a real piece (not NO_PIECE). always_inline: called from the
  // make/unmake primitives (put_piece/remove_piece/move_piece), the hottest path in the engine.
